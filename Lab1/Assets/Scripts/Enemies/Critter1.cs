...
public class Critter1 : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("Bullet"))
        {
            ...
            Destroy(gameObject);
            AudioManager.Instance.PlayModifiedSound(AudioManager.Instance.zap);
            GameManager.Instance.critterCounter++;
        }
        else if(collision.gameObject.CompareTag("Player"))
        {
            Instantiate(heatEffect, transform.position, transform.rotation);
            Destroy(gameObject);
            AudioManager.Instance.PlayModifiedSound(AudioManager.Instance.heat);
            GameManager.Instance.critterCounter++;
        }
    }
}
